        ...
        for (int row = 0; row < field.getDepth(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                if (RANDOM.nextDouble() <= FOX_CREATION_PROBABILITY) {
                    ...
                    Animal animal = AnimalFactory.createAnimal(AnimalType.FOX, true, location, field);
                    foxes.add(animal);
                } else if (RANDOM.nextDouble() <= RABBIT_CREATION_PROBABILITY) {
                    Location location = new Location(row, col);
                    Animal animal = AnimalFactory.createAnimal(AnimalType.RABBIT, true, location, field);
                    rabbits.add(animal);
                }
